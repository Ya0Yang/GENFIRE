...
function obj = fillInFourierGrid_FFT_tri(obj)

projections = obj.InputProjections;
angles = obj.InputAngles;
projectionSize = obj.particleWindowSize(2);
...
padding = round(projectionSize*(oversamplingRatio-1)/2);
centralPixel = size(projections,2)/2+1;
halfWindowSize = projectionSize/2;

%initialize array to hold measured data
if mod(projectionSize,2)==0
    kMeasured = zeros(floor(projectionSize*oversamplingRatio),floor(projectionSize*oversamplingRatio),size(projections,3),'single');
else
    kMeasured = zeros(1 + floor((projectionSize-1)*oversamplingRatio),1 + floor((projectionSize-1)*oversamplingRatio),size(projections,3),'single');
end
...
% kernel smoothing
%masterDistances = exp(-sigma^2*masterDistances);
